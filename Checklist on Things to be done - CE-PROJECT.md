- [x] Finish the homepage design
- [ ] about page
- [ ] contact us page
- [ ] FAQs
- [x] Loading state sa Login page
- [x] Loading state sa Registration page
- [x] profile page para sa mga users
- [x] Checkbox para sa "use your own email" (account setup nga page)
- [ ] Forgot password nga feature
	
### ADMIN POV
- [x] Dynamic data sa admin dashboard
- [x] Filter user feature sa user management
- [x] Empty state sa user management page
- [x] Empty state sa worker management
- [x] Progress info sa job details
- [x] Filter feature sa job management
- [x] Account activation through email para sa worker	


### Client POV
- [x] Dynamic data sa client dashboard
	- [x] Recent job requests
	- [x] Recent activities
- [ ] Empty state para sa "My Jobs nga page"
- [x] Validation errors para sa create job
- [ ] Loading state para sa create job
- [x] progress info sa job details (client)
- [x] Delete Job
- [x] kailangan maka reject ug marked as completed nga job
- [x] Maka approve ug marked as completed nga job
- [x] Dapat inig ma completed ang job, ma addan ang earnings sa admin
- [ ] Ma notify inig naay ma assign sa job request

### WORKER POV
- [ ] Dynamic data sa client dashboard
	- [x] Active jobs pila ka job iyang gi handle
	- [x] Completed jobs pila iyang overall completed jobs
	- [x] Rating
	- [x] Pending assigned jobs
	- [x] Recent Activities
- [x] List sa iyang mga assigned jobs
- [x] Job details nga page
- [x] Sa job details didto siya mag butang sa iyang progress
- [x] View progress detail
- [x] Mark as complete ang job



- [ ] Email notification

